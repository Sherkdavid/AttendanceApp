...
CREATE TABLE IF NOT EXISTS `db`.`faculty` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `lecturer_id` (`id` ASC),
...
CREATE TABLE IF NOT EXISTS `db`.`course` (
  ...
  PRIMARY KEY (`id`),
  INDEX `lecturer_id` (`department` ASC),
  CONSTRAINT `department`
    FOREIGN KEY (`department`)
    REFERENCES `db`.`department` (`name`)
    ON DELETE NO ACTION
...
CREATE TABLE IF NOT EXISTS `db`.`class` (
  ...
  INDEX `class_id` (`id` ASC),
  CONSTRAINT `lecturer`
    FOREIGN KEY (`lecturer_id`)
    REFERENCES `db`.`faculty` (`id`),
  CONSTRAINT `module`
    FOREIGN KEY (`module_id`)
    REFERENCES `db`.`module` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `db`.`student` (
  ...
  `email` VARCHAR(255) NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `student_id` (`id` ASC),
